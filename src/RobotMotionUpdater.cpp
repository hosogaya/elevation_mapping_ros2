...
bool RobotMotionUpdater::update(ElevationMap& _map, const PoseTransform& _pose_transform, const PoseCovariance& _pose_covariance, const rclcpp::Time& _time_stamp)
{
    ...
    for (size_t i=0; i<static_cast<size_t>(index(0)); ++i)
    {
        for (size_t j=0; j<static_cast<size_t>(index(1)); ++j)
        {
            ...
            else 
            {
                ...
                horizontal_variance_update_x = std::numeric_limits<float>::infinity();
                horizontal_variance_update_y = std::numeric_limits<float>::infinity();
                horizontal_variance_update_xy = std::numeric_limits<float>::infinity();
            }
        }
    }
...
bool RobotMotionUpdater::computeReducedCovariance(const PoseTransform& _pose_transform, const PoseCovariance& _pose_covariance, ReducedCovariance& _reduced_covariance)
{
   ...
   Eigen::EulerAnglesZYXd euler_zyx = _pose_transform.rotation().eulerAngles(2, 1, 0);
   double tan_pitch = std::tan(euler_zyx.beta());

   Eigen::Matrix<double, 1, 3> yaw_jac(std::cos(euler_zyx.alpha())*tan_pitch, std::sin(euler_zyx.alpha())*tan_pitch, 1.0);
...
bool RobotMotionUpdater::computeRelativeCovariance(const PoseTransform& _pose_transform, const ReducedCovariance& _reduced_covariance, ReducedCovariance& _relative_covariance)
{
    ...
    euler_zyx.beta() = 0.0;
    euler_zyx.gamma() = 0.0;
    Eigen::Matrix3d R_I2tildeB = euler_zyx.toRotationMatrix();

    // velocity*delta_t
    Eigen::Vector3d v_delta_t = pre_robot_pose_.rotation().transpose()*(_pose_transform.translation() - pre_robot_pose_.translation());    

    Eigen::Matrix<double, 4, 1> f; // F
    f.setIdentity();
    f.topRightCorner(3, 1) = computeSkewMatrixfromVector(Eigen::Vector3d::UnitZ()) * R_I2tildeB*v_delta_t;

    // jacobina inv(G)*delta_t
    Eigen::Matrix<double, 4, 1> inv_g_delta_t;
    inv_g_delta_t.setZero();
    inv_g_delta_t(3,3) = 0.0;
    Eigen::Matrix<double, 4, 1> inv_g_transpose_delta_t(inv_g_delta_t);
...
void RobotMotoinUpdater::readParameters(rclcpp::Node* _node)
{
